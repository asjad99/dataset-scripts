...
#globals
CONSUMER_KEY, CONSUMER_SECRET = get_app_token('Scope')
...
class RateLimitThread(threading.Thread):
    def __init__(self,user):
        ...
        self.reply = check_rate_limit(api,'friends,followers,users')
...
class TwitterHandle(object):
    ...
    #---------------------------------------
    def is_valid(self):
        reply = check_rate_limit(self.api, self.resource)
...
def check_rate_limit(api, check_resources='none'):
    ...
    for i in resources:
        ...
        if(i == 'friends'):
            ...
            temp = {}
            temp['resource'] = i 
            temp['status'] = convert_to_min(status['resources']['users']['/users/lookup'])
            reply.append(temp)  
...
def debug():
    ...
    reply = twitter_auth.users.find_one({'twitter_handle':'neebi'})
    api = perform_tweepy_auth(reply['access_key'],reply['access_secret']) 
    pretty_print(check_rate_limit(api,'friends,followers,users'))
